import pytest
import numpy as np 
from selective_inference.lasso_si import parametric_lasso_si,parametric_lasso_ci,parametric_lasso_cv_si
...
def test_lasso_si(data_generate,p_test,A_test):
    p_values,A = parametric_lasso_si(data_generate[0],data_generate[1],0.01)
...
def test_lasso_cv_si(data_generate,p_test,A_test,k_test):
    p_values,A,k = parametric_lasso_cv_si(data_generate[0],data_generate[1],np.geomspace(0.001,10,5).tolist(),5)
    np.testing.assert_allclose(p_values,p_test)
    assert A == A_test
    assert k == k_test