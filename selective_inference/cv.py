...
def validation_error(model,X_train,X_val,y_train,y_val):

    X_train_A = X_train[:,model]
    X_val_A = X_val[:,model]
...
def cv_error(model,X,y,k_cv):
    ...
    for i in range(k_cv):
        ...
        error_array[i] = validation_error(model,X_train,X_val,y_train,y_val)
